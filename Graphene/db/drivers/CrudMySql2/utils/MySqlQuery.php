...
class MySqlQuery {
    ...
    private static $SELECT_QUERY_MODEL = 'SELECT <selection> FROM <identifier> WHERE <cond> <paging>';
    ...
    public static function getReadQueryComponents($settings, $request) {
        return [
            'model'     => self::$SELECT_QUERY_MODEL,
            'selection' => '*',
            'from'      => self::getSelect($request, $settings),
    ...
    private static function getCondition($request) {
        ...
        if (count($request->getModel()->getFlatDbValues()) > 0 && /*!$request->hasSearch() &&*/
            !$request->hasWhere()
        ) {
            $clauses = ' AND';
