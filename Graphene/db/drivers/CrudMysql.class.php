<?php
    ...
    class CrudMySql implements CrudDriver {
        ...
        public function read($json,$query = null) {
            $prefs = array_key_exists('prefs',$query) ? $query['prefs'] : null;
            $this->init($json);
            $decoded = json_decode($json,true);
            $q = self::SELECT_PTT;
            ...
            if ($res instanceof PDOStatement) {
                ...
                foreach ($results as $res) {
                    $return[] = $this->colsToJsonArr($res,$json,$prefs);
                }
            }
        ...
        private function getCondition($query,$cols) {
            ...
            if (array_key_exists('order',$query)) {
                ...
                if (array_key_exists('mode',$query['order'])) {
                    $cond = $cond . 'ASC';
                } else {
                    $cond = $cond . 'DESC';
                }
         ...
         * @return array
         */
        private function colsToJsonArr($row,$json,$prefs) {
            $res = [];
            ...
            foreach ($row as $k => $v) {
                ...
                if (count($expl) > 1) {
                    ...
                    $tRes[$k] = $this->getSerializedValue($v,$tStruct[$k],$prefs);
                }
            }
        ...
        private function getSerializedValue($value,$definition,$prefs) {
            if (Strings::contains($definition,Model::DATETIME)) {
                if ($value === '0000-00-00 00:00:00') {
                    $value = null;
                } else {
                    if (array_key_exists('resultSerialization',$prefs) &&
                        array_key_exists('dateFormat',$prefs['resultSerialization'])
                    ) {
                        $value = date($prefs['resultSerialization']['dateFormat'],new \DateTime($value));
                    } else {
                        //$value = date('Y-m-d H:i:s',new \DateTime($value));
                        $value = date(DATE_ISO8601,new \DateTime($value));

                    }
                }
            } else if (Strings::contains($definition,Model::BOOLEAN)) {
                if ($value === 1 || $value === '1') {
                    $value = true;
                } else {
                    $value = false;
                }
            } else if (Strings::contains($definition,Model::INTEGER) && ($value !== null || $value !== '')) {
                $value = intval($value);
            } else if (Strings::contains($definition,Model::DECIMAL) && ($value !== null || $value !== '')) {
