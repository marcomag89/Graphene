...
class Filter {
	public function setUp($ownerModule, $attributes) {
		if (!isset($attributes['scope']) == null || strcasecmp($attributes['scope'], 'local')==0) 	$this->$scope = 'local';
		else if(strcasecmp($attributes['scope'], 'global')==0) $this->$scope = 'global';
		
		$this->actions = array ();
		$this->modules = array ();
		
		if(isset($attributes['actions'])){$this->actions = explode(',', str_replace(' ', '', $attributes['actions']));}
		if(isset($attributes['modules'])){$this->actions = explode(',', str_replace(' ', '', $attributes['modules']));}
	}
