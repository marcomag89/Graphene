...
class GroupPermissionRead extends Action {
    ...
    public function run() {
        ...
        if ($groupName === Group::$superUserGroupName) {
            $this->response->setBody(json_encode(["PermissionSet" => ["group" => $groupName, "permissions" => ['*']]], JSON_PRETTY_PRINT));
            return;
        }
        do {
        ...
        $this->response->setBody(json_encode(["PermissionSet" => ["group" => $groupName, "permissions" => $retPermissions]], JSON_PRETTY_PRINT));
    }
}