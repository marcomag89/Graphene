...
class Create extends Action
{
    ...
    public function run()
    {
        ...
        if (! $user instanceof User)
            throw new GraphException('invalid user', 4001, 400);
        if (! $user->checkPassword()) {
            $this->sendError('400', 'Password requires one upper and one number 6-25 chars');
            return;
        }
        ...
        try {
            ...
            if ($e->getCode() == '3100')
                throw new GraphException('User: ' . $user->getEmail() . ', already exists', 4002, 400);
            else
