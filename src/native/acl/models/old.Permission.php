...
class PermissionOld extends Model {
    ...
    public function securityChecks() {
        ...
        if ($this->getGroup() === Group::$superUserGroupName) {
            throw new GraphException('cannot edit ' . Group::$superUserGroupName . ' group permissions', 300);
        }

        //Check if already assigned
        if ($this->read() !== null) {
            throw new GraphException('permission ' . $this->getAction() . ' already assigned at ' . $this->getGroup(), 400);
        }
    }
