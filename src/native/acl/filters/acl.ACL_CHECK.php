<?php
    ...
    class AclCheck extends Filter {
        ...
        public function run() {
            ...
            if ($appInfo !== null && array_key_exists('permissions',$appInfo))
        ...
        private function loadAppInfo($apiKey) {
            ...
            if ($apiKey !== null) {
                $result = $this->forward('/acl/app/withPermission/' . $apiKey)->getData();
                self::$cache['app-info'][$apiKey] = array_key_exists('App',$result) ? $result['App'] : null;
            }
            return self::$cache['app-info'][$apiKey];
        }
        ...
        private function loadPermissions($user) {
            if ($user === null)
                $userId = 'anonimous'; else $userId = $user['id'];
            if (array_key_exists($userId,self::$cache['permissions']))
        ...
        private function isAclEnabled() {
            if (array_key_exists('bool',self::$cache['enabled']))
